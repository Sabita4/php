...
<form action="{{route('categories.store')}}" method="POST">
            @csrf
            <input type="text" class="border border-gray-300 p-2 rounded-md w-full mb-3" name="order" placeholder="Order" value="{{old('order')}}">
            @error('order')
            <div class="text-red-500 mb-3 -mt-3">{{$message}}</div>
            @enderror
            <input type="text" class="border border-gray-300 p-2 rounded-md w-full" name="name" placeholder="Category Name" value="{{old('name')}}">
            @error('name')
